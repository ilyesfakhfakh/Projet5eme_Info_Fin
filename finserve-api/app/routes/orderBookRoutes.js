...
router.get('/order-book', async (req, res) => {
  ...
  const buyOrders = await require('../services/order-book.service').getOrders('BUY', { portfolioId, assetId })
  const sellOrders = await require('../services/order-book.service').getOrders('SELL', { portfolioId, assetId })
