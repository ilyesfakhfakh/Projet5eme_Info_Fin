...
function MenuList() {
  ...
  const filtered = menuItems.items
    .map((g) => filterMenuByPermissions(g, user))
    .filter((g) => g && Array.isArray(g.children) && g.children.length > 0);

  const navItems = filtered.slice(0, lastItemIndex + 1).map((item, index) => {
    switch (item.type) {
      case 'group':
